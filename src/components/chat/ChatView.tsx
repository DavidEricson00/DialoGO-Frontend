...
export default function ChatView({ chatId, onBack, onLeave, onDelete }: ChatViewProps) {
  ...
  const isOwner = !!currentUser && !!chat && String(currentUser.id) === String(chat.owner_id);
  ...
  return (
    <>
      ...
      <GenericModal
        ...
        title="Excluir Chat"
        description={<>Tem certeza que deseja excluir permanentemente o chat <strong>{chat?.name}</strong>? Esta ação não poderá ser desfeita.</>}
        type="warning"
        confirmText="Excluir"
        cancelText="Cancelar"
